...
pin<Method> Ast::mk_overload(pin<Class> cls, pin<Method> ovr) {
	auto m = pin<ast::Method>::make();
	m->mut = ovr->mut;
	m->name = ovr->name;
	m->cls = cls;
...
pin<ast::Function> Ast::mk_fn(string name, pin<Action> result_type, std::initializer_list<pin<Type>> params) {
