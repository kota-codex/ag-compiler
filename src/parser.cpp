...
struct Parser {
	...
	pin<ast::Module> parse(module_text_provider_t module_text_provider, int64_t requested_version)
	{
		...
		if (auto it = ast->modules.find(module_name); it != ast->modules.end()) {
			if (it->second->version < requested_version) {
				// TODO: reload and restart
				error("incompatible module versions. Module", module_name, " first resolved to version ", it->second->version, " then imported as ", requested_version);
			}
			return it->second;
		}
		module = new ast::Module;
		module->name = module_name;
		module->version = requested_version;
		text = module_text_provider(module_name, module->version, module->path);
		if (module_name != "sys")
		...
		while (match("using")) {
			...
			if (is_num(*cur)) {
				...
				if (auto v = get_if<uint32_t>(&*n))
					using_version = *v;
				else if (auto v = get_if<uint64_t>(&*n))
					using_version = *v;
				else
					error("expected intefer version");
			}
